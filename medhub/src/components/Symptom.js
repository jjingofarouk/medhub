...
const SymptomChecker = () => {
    const [symptoms, setSymptoms] = useState({ symptom1: '', symptom2: '', symptom3: '' });
    const [results, setResults] = useState([]);
    const [error, setError] = useState('');
  
    const handleChange = (e) => {
        setSymptoms({ ...symptoms, [e.target.name]: e.target.value.trim().toLowerCase() });
    };
  
    const checkSymptoms = (e) => {
        e.preventDefault();
  
        const userSymptoms = Object.values(symptoms).filter(symptom => symptom);
        ...
        const diagnosis = calculateConditionScores(userSymptoms);
            ...
            <main className="main-content">
                ...
                <form onSubmit={checkSymptoms}>
                    <div className="form-group">
                        ...
                        <input type="text" id="symptom1" name="symptom1" onChange={handleChange} />
                    </div>
                    <div className="form-group">
                        <label htmlFor="symptom2">Symptom 2:</label>
                        <input type="text" id="symptom2" name="symptom2" onChange={handleChange} />
                    </div>
                    <div className="form-group">
                        <label htmlFor="symptom3">Symptom 3:</label>
                        <input type="text" id="symptom3" name="symptom3" onChange={handleChange} />
                    </div>
                    {error && <p className="error">{error}</p>}
                    <button type="submit" className="button">Check Symptoms</button>
                </form>
                ...
                {results.length > 0 && (
                    <div id="results">
                        ...
                        <ul id="diagnosisList">
                            {results.map(result => (
                                <li key={result.condition}>
                                    <strong>{result.condition}</strong> (Score: {result.score})
                                    {guidance[result.condition] && (
                                        <>
                                            <p><strong>Next Steps:</strong> {guidance[result.condition].steps}</p>
                                            <p><strong>Educational Content:</strong> {guidance[result.condition].content}</p>
                                        </>
                                    )}
                                </li>
                            ))}
